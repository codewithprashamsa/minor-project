...
% ─── TikZ styles ────────────────────────────────────────────────────────────
\tikzset{
  block/.style={rectangle, draw, fill=gray!10, rounded corners,
                text width=2.8cm, text centered, minimum height=1cm, font=\small},
  wideblock/.style={rectangle, draw, fill=gray!10, rounded corners,
                text width=3.8cm, text centered, minimum height=1cm, font=\small},
  decision/.style={diamond, draw, fill=gray!20, text width=2.2cm,
                   text centered, inner sep=0pt, font=\small},
  arrow/.style={-{Stealth[length=2mm]}, thick},
  greenblock/.style={rectangle, draw, fill=gray!15, rounded corners,
                text width=2.8cm, text centered, minimum height=1cm, font=\small},
  redblock/.style={rectangle, draw, fill=gray!25, rounded corners,
                text width=2.8cm, text centered, minimum height=1cm, font=\small},
  % --- Layer-type styles for CNN architecture diagram ---
  convlayer/.style={rectangle, draw=black!70, fill=gray!15, rounded corners=3pt,
                    text width=3.2cm, text centered, minimum height=0.85cm,
                    font=\small\bfseries},
  normlayer/.style={rectangle, draw=black!60, fill=gray!10, rounded corners=3pt,
                    text width=3.2cm, text centered, minimum height=0.7cm,
                    font=\small},
  poollayer/.style={rectangle, draw=black!60, fill=gray!20, rounded corners=3pt,
                    text width=3.2cm, text centered, minimum height=0.7cm,
                    font=\small},
  droplayer/.style={rectangle, draw=black!60, fill=gray!8, rounded corners=3pt,
                    text width=3.2cm, text centered, minimum height=0.7cm,
                    font=\small},
  denselayer/.style={rectangle, draw=black!70, fill=gray!15, rounded corners=3pt,
                     text width=3.2cm, text centered, minimum height=0.85cm,
                     font=\small\bfseries},
...
and a softmax confidence-thresholding rejection layer. The system operates
exclusively on the unencrypted TLS Client\,Hello---without decrypting any
payload---and uses JA4 fingerprints (with automatic JA3 fallback) encoded as
fixed-length numerical vectors for classification. Flows without a TLS handshake
are excluded from the pipeline.

A softmax cutoff flags out-of-distribution traffic and routes it to analysts,
eliminating the closed-set fallacy under which novel malware families are silently
misclassified. The system is trained on known malware families and evaluated on
an entirely unseen family to demonstrate open-set detection capability. Class
imbalance is addressed through inverse-frequency class weighting. Two baselines
are compared: Random Forest on the same JA4 feature vectors and a JA3-CNN.
Dropout and early stopping mitigate overfitting on the short feature vectors.

\vspace{0.1cm}
\noindent\textbf{Keywords:} Malware Traffic Classification, TLS Fingerprinting,
JA4, 1D-CNN, Open-Set Recognition, Softmax Thresholding, Encrypted
Traffic Analysis
...
Current encrypted malware detection systems suffer from two compounding weaknesses.
First, deep learning classifiers trained under the closed-set assumption cannot flag
genuinely unknown traffic; novel malware families are silently misclassified as the
nearest known family, generating critical false negatives~\cite{bendale2016openmax,
opensetmalware2023}. Second, much prior work relies on the JA3 fingerprinting
standard, which is vulnerable to cipher-suite reordering and GREASE extension
randomisation~\cite{althouse2023ja4}. When a novel malware family appears in a live
network, these systems create a prolonged dwell time that can allow advanced threats
to persist for months~\cite{ibm2023breach}, while overly aggressive classifiers that
force unknown traffic into known categories generate excessive false positives,
overwhelming SOC analysts with alert fatigue.

\section{Objectives}

\begin{itemize}[leftmargin=*]
  \item Develop an open-set malware traffic classification system using JA4 fingerprinting and 1D-CNNs.
  \item Classify known malware families and flag unseen ones without decrypting TLS communications.
  \item Implement softmax confidence-thresholding for open-set rejection of unknown traffic.
  \item Compare classification performance against Random Forest and JA3-CNN baselines.
\end{itemize}
...
Ten representative papers were reviewed across five thematic areas: TLS fingerprinting, deep learning for traffic classification, open-set recognition in network security, class imbalance strategies, and overfitting mitigation for short feature vectors.

\section{TLS Fingerprinting for Encrypted Traffic Analysis}

TLS fingerprinting has emerged as a primary technique for identifying applications and detecting malware in encrypted traffic without decryption. Anderson and McGrew (2017) demonstrated the efficacy of using passive, unencrypted TLS handshake metadata with a Random Forest classifier on a large proprietary dataset, achieving high detection rates by analyzing cipher suites, extensions, and SNI. However, their system operated under a closed-set assumption and remains vulnerable to active mimicry by sophisticated attackers who can spoof handshake metadata.

Sirinam et al. (2018) investigated the limits of TLS handshake fingerprinting (focusing on JA3) combined with Random Forest and SVM classifiers. Their evaluation showed that ML models could identify applications with high precision based only on Client Hello metadata, but the study was constrained by closed-set evaluation and relied on the JA3 standard, which is easily bypassed by modern cipher-suite randomization techniques. Burgetov\'{a} et al. (2025) examined the synergistic effect of combining the newer JA4+ fingerprinting standard with traditional flow-level statistical features using a hybrid ML approach on the CESNET TLS dataset, showing that fusing application-layer fingerprints with transport-layer features yields superior performance, though they did not explore deep learning architectures or open-set detection. L\'{o}pez de Vergara et al. (2025) presented an empirical comparison between JA4+ and traditional flow-level statistics (using the MalDIST tool), concluding that JA4+ is highly competitive, especially for identifying targeted client-side threats, but is limited when malware mimics legitimate browsers, suggesting the need for multi-modal verification or advanced thresholding.

\section{Deep Learning for Encrypted Traffic Classification}

Deep learning has shown strong performance on sequence-structured network data. Wang et al. (2017) proposed applying 1D-CNNs directly to raw network packet bytes, treating traffic as a sequence of bytes. This end-to-end method bypassed manual feature engineering and outperformed classical ML baselines on the ISCX VPN-nonVPN dataset. However, processing raw byte sequences is highly resource-intensive, requires deep packet inspection, and the classification was limited to a closed-set environment.

Liu et al. (2024) introduced a semi-supervised multimodal deep learning model combining a 1D-CNN with autoencoders to address class imbalance and lack of labeled data, achieving 99.41\% F1-score on UNSW-NB15 using a fraction of labeled samples, but remained within a closed-set structure. A later multi-modal 1D-CNN framework by Liu et al. (2025) took both raw packet bytes and header flow statistics as input branches through an encoder-decoder architecture, proving robust against payload obfuscation on USTC-TFC2016 and ISCX datasets, yet maintained a strict closed-set training environment. The SPDConv-Enhanced 1D-CNN (2025) introduced Space-to-Depth Convolution blocks to accelerate inference on sequential network data, minimizing latency on network traffic datasets without degrading accuracy, but did not evaluate open-set scenarios or incorporate TLS fingerprinting.

\section{Open-Set Recognition in Network Security}

The closed-set fallacy---where classifiers force every test sample into known classes---remains a fundamental limitation in deployed systems. Bendale and Boult (2016) introduced OpenMax, extending deep networks to open-set recognition using Extreme Value Theory (EVT) to calibrate activation vectors and fit Weibull distributions for threshold-based rejection of unknown classes. While demonstrated on ImageNet with a 4.3\% accuracy gain over softmax thresholding, adapting EVT-calibrated OpenMax to network traffic analysis remains challenging and relatively unexplored.

Chen et al. (2023) proposed an uncertainty-aware deep learning framework for open-set network traffic classification using predictive uncertainty estimation, successfully flagging unknown traffic flows in proprietary datasets. However, the model does not exploit TLS fingerprinting standards (JA4/JA3) and requires complex probability calibration during inference. These works collectively highlight that open-set recognition for network traffic is an active area, but the integration with modern TLS fingerprinting standards remains unexplored.

\section{Class Imbalance Strategies}

Real-world malware datasets are heavily skewed. Liu et al. (2024) addressed this through semi-supervised learning with autoencoders, achieving high F1-scores with limited labeled data. A more direct approach for supervised settings is cost-sensitive learning. The Group \& Reweight method (2024) introduced a novel cost-sensitive approach to mitigate class imbalance in network traffic classification, showing that inverse-frequency class weighting achieves the same gradient-level upsampling effect without generating synthetic samples that may produce physically impossible TLS configurations. This aligns with the observation that SMOTE interpolation in feature space can create JA4 vectors with cipher-suite counts inconsistent with any real TLS implementation.

\section{Overfitting Mitigation for Short Feature Vectors}

JA4 fingerprint vectors are short (approximately 20--40 dimensions after encoding), creating a high model-capacity-to-input ratio that leads to rapid memorization in deep architectures. Standard countermeasures include dropout in the fully-connected head and early stopping on validation loss. Srivastava et al. (2014) established dropout as a simple way to prevent overfitting, while Prechelt (2012) formalized early stopping with patience-based criteria. These techniques are well-understood and directly applicable to the 1D-CNN architecture proposed in this work.

Table~\ref{tab:litreview} presents all reviewed papers in a unified tabular format.
...
profile from a browser, a Python script, or a Cobalt Strike beacon. Publicly available network traffic datasets include TLS flows from both benign HTTPS traffic and botnet command-and-control sessions. Flows that carry no TLS handshake (pure UDP,
...
vision domains (row 3) or lack TLS fingerprinting entirely (row 10).
JA4-based studies are either classification-free (row 7) or closed-set
...
for 1D-CNN construction. Publicly available network traffic datasets are widely used in
the research community and provide genuine TLS handshake captures for evaluation. Training on a mid-range GPU (e.g., NVIDIA RTX 3060,
12\,GB VRAM) or free-tier Google Colab is sufficient for the dataset size involved.

\subsection{Economic Feasibility}
All software dependencies are free and open-source. Publicly available network traffic datasets are freely
downloadable from academic repositories. Cloud compute (Google Colab or Kaggle) provides GPU access at minimal
cost.
...
\begin{tikzpicture}[
  ...
  usecase/.style={draw, ellipse, minimum width=3.2cm, minimum height=0.8cm, align=center, fill=gray!5, font=\small, thick},
...
\centering
\begin{tikzpicture}[
  block/.style={draw, rectangle, minimum width=3.5cm, minimum height=1.2cm, align=center, fill=gray!5, font=\small, thick, rounded corners=3pt},
...
\centering
\begin{tikzpicture}[
  startstop/.style={draw, rectangle, rounded corners=10pt, minimum width=2cm, minimum height=0.8cm, align=center, fill=gray!15, thick},
  process/.style={draw, rectangle, minimum width=3cm, minimum height=0.8cm, align=center, fill=gray!5, thick},
  decision/.style={draw, diamond, aspect=2, minimum width=2.5cm, minimum height=1cm, align=center, fill=gray!10, thick},
  arrow/.style={-{Stealth[length=2.5mm]}, thick}
]
...
\end{tikzpicture}
\caption{Proposed 1D-CNN Architecture}
\label{fig:cnn_arch}
\end{figure}
...
\begin{tikzpicture}[node distance=0.2cm and 0.6cm, scale=0.58]
  \node[wideblock] (s1) {\textbf{Step 1}\\ Download Network Traffic\\ Dataset; verify TLS coverage};
...
A single publicly available network traffic dataset is used for all experiments, capturing realistic network traffic across multiple attack types including botnet activity. Raw PCAP files are publicly available, allowing genuine JA4 extraction from packet captures rather than pre-computed flow statistics. TLS handshake coverage is verified and reported per malware family; flows lacking a complete handshake are excluded.
...
For the step-by-step data collection, raw PCAP files are downloaded from the Stratosphere Malware Capture Facility Project (CTU-Malware-Capture-Botnet series) hosted at \url{https://www.stratosphereips.org/datasets-malware}. This dataset provides long-term captures of real botnet traffic with per-flow family labels (Emotet, TrickBot, Zeus, and others) alongside normal background traffic. SHA-256 checksums are verified to ensure data integrity. All PCAP files are scanned to count flows with and without TLS handshakes, reporting TLS coverage per family label. This confirms whether sufficient fingerprint-able traffic exists for each class.
...
\begin{ganttchart}[
    ...
    group top shift=0.4,
    group height=0.25,
    bar/.append style={fill=gray!30, draw=gray!60},
    group/.append style={fill=gray!20, draw=gray!50},
    milestone/.append style={fill=gray!50, draw=gray!70},
    title height=1
 ]{1}{8}
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  \item \textbf{JA4 extraction pipeline} that processes network traffic PCAPs and
...
\bibitem{stratosphere2024}
Stratosphere Laboratory, ``Malware Capture Facility Project: CTU-Malware-Capture-Botnet,''
Czech Technical University, 2024.
[Online]. Available: \url{https://www.stratosphereips.org/datasets-malware}

\bibitem{garcia2014}
S.~Garcia \textit{et al.}, ``An Empirical Comparison of Botnet Detection Methods,''
\textit{Computers \& Security}, vol.~45, pp.~100--123, 2014.
